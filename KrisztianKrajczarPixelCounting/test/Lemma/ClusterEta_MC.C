...
//  gStyle->SetOptStat(0);
  ...
  for(int i = 1; i<=11; i++) {
    ntupleMC.Add(Form("crab_0_101105_085004/res/test_sim_%i_1.root",i));
  }

  hClusterEta = new TH1F("hClusterEta","hClusterEta",360,-4.,4.);
  hClusterEtaMC = new TH1F("hClusterEtaMC","hClusterEtaMC",360,-4.,4.);

  ntuple->Project("hClusterEta","eta","rver_z!=0 && bit34 && isTrigger_1 && bit36==0 && bit37==0 && bit38==0 && bit39==0");
  ntupleMC->Project("hClusterEtaMC","eta","bit34 && isTrigger_1 && rver_z!=0");

  hClusterEta->Sumw2();

  hClusterEtaMC->GetXaxis()->SetTitle("#eta");
  hClusterEtaMC->GetYaxis()->SetTitle("Probability");

  hClusterEta->SetMarkerStyle(20);
//  hClusterEta->SetMarkerSize(0.1);
  hClusterEta->SetMarkerColor(1);

  hClusterEta->Scale(1./hClusterEta->Integral());
  hClusterEtaMC->Scale(1./hClusterEtaMC->Integral());

//  hClusterEta->SetMaximum(0.08);
  hClusterEta->SetNdivisions(202);


  hClusterEtaMC->SetLineColor(2);
  hClusterEtaMC->Draw();
  hClusterEta->Draw("Psame");

  TLegend *leg6 = new TLegend(0.25,0.55,0.85,0.65,NULL,"brNDC");
  leg6->AddEntry(hClusterEta,"Data, event selection","P");
  leg6->AddEntry(hClusterEtaMC,"MC, event selection","L");
