...
// reason: binning in multiplicity?
...
//   dndeta0->GetYaxis()->SetTitleOffset(0.89);
  ...
  for(int i = 1; i<=11; i++) {
    ntuple1.Add(Form("crab_0_101105_085004/res/test_sim_%i_1.root",i));
  }

  TChain ntuple2("multProp_looper");
  for(int i = 1; i<=11; i++) {
    ntuple2.Add(Form("crab_0_101105_085004/res/test_sim_%i_1.root",i));
  }

  //sim
  TChain ntuple1_sim("multProp");
  for(int i = 1; i<=11; i++) {
    ntuple1_sim.Add(Form("crab_0_101105_085004/res/test_sim_%i_1.root",i));
  }

  TChain ntuple2_sim("multProp_looper");
  for(int i = 1; i<=11; i++) {
    ntuple2_sim.Add(Form("crab_0_101105_085004/res/test_sim_%i_1.root",i));
  }

  TChain ntuple3_sim("hJustLooper");
  for(int i = 1; i<=11; i++) {
    ntuple3_sim.Add(Form("crab_0_101105_085004/res/test_sim_%i_1.root",i));
  }
...
//     std::cerr<<" mult_bin_lemma: " << mult_bin_lemma << std::endl;
  ...
  for(int i = 0; i < num_mult_bins-1; i++) {   //-1 because [i+1]
     ...
     //mc
     sprintf(cond,"centBin==2 && cluster_counter>=%i && cluster_counter<%i",mult_bins[i],mult_bins[i+1]);
     sprintf(condV,"centBin==2 && rver_z!=0 && bit34 && isTrigger_1 && bit36==0 && bit37==0 && bit38==0 && bit39==0 && cluster_counter>=%i && cluster_counter<%i",mult_bins[i],mult_bins[i+1]);

     //data
     sprintf(condVData,"centBin==2 && rver_z!=0 && bit34 && isTrigger_1 && bit36==0 && bit37==0 && bit38==0 && bit39==0 && cluster_counter>=%i && cluster_counter<%i",mult_bins[i],mult_bins[i+1]);
...
*/
  ...
  for(int i = 1; i<=11; i++) {
    hSim_lemma->Reset();
    TFile f(Form("crab_0_101105_085004/res/test_sim_%i_1.root",i));

    hSim_lemma = (TH1D*)f.Get("hTestEtaFull_2");
    hSim->Add(hSim_lemma);
  }

  hSim->Scale(1./(30.*hSim->GetBinWidth(0)));
  hSim->SetLineColor(2);
  hSim->Draw("same");
...
//  leg6->Draw();
  ...
  MyCanvas.SaveAs("HI_MC_layer1_bin2_LOCAL_plot.C");
}
