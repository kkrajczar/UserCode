...
//  gStyle->SetOptStat(0);
  ...
  TChain ntuple("multProp");
  ntuple.Add("HIatom_test_large.root");

  TChain ntupleMC("multProp");
  ntupleMC.Add("HIatom_test_large.root");

  hChargeDist = new TH1F("hChargeDist","hChargeDist",360,0.,18000.);
  hChargeDistMC = new TH1F("hChargeDistMC","hChargeDistMC",360,0.,18000.);

  ntuple->Project("hChargeDist","clusters_on_layers","rver_z!=0 && bit34 && isTrigger_1 && bit36==0 && bit37==0 && bit38==0 && bit39==0");
  ntupleMC->Project("hChargeDistMC","clusters_on_layers","bit34 && isTrigger_1 && rver_z!=0");
