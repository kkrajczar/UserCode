...
#)
process.analyzeTracks = cms.EDAnalyzer("ChargedMultiplicityAnalyzerNoSimInfo_new",
    src = cms.InputTag("siPixelRecHits"),
    file = cms.string('test_NOSIM.root'),
    simulateTriggers = cms.bool( False ),
    produceVertexLocally = cms.bool( True ),
    vertexColl = cms.InputTag("clusterVertices")
)

process.clusterVertices = cms.EDFilter("VertexProducerClusterCounting")
...
#process.analyze_step = cms.Path(process.clusterVertices*process.analyzeTracks)
...
#process.schedule = cms.Schedule(process.beamS,process.raw2digi_step,process.reconstruction_step,process.analyze_step)#,process.endjob_step)
process.schedule = cms.Schedule(process.raw2digi_step,process.reconstruction_step,process.analyze_step)#,process.endjob_step)
