...
*/
   ...
   TH1D * hJetPt_minbias = new TH1D("hJetPt_minbias","hJetPt_minbias",ptBins.size()-1,&ptBins[0]);
   TH1D * hJetPt_larger_minbias = new TH1D("hJetPt_larger_minbias","hJetPt_larger_minbias",ptBins_larger.size()-1,&ptBins_larger[0]);
...
//   TH1D * hPartPt_minbias = new TH1D("hPartPt_minbias","hPartPt_minbias",ptBins_part.size()-1,&ptBins_part[0]);
...
//   TH1D * hPartPt_0_40_minbias = new TH1D("hPartPt_0_40_minbias","hPartPt_0_40_minbias",ptBins_part.size()-1,&ptBins_part[0]);
   ...
   TH1D * hNumEv_95_120_minbias = new TH1D("hNumEv_95_120_minbias","hNumEv_95_120_minbias",1,0.,1.);
   TH1D * hNumEv_120_X_minbias = new TH1D("hNumEv_120_X_minbias","hNumEv_120_X_minbias",1,0.,1.);

   TH1D * hRecMultiplicity = new TH1D("hRecMultiplicity","hRecMultiplicity",300.,0.,300.);

   hJetPt_minbias->Sumw2();
   hJetPt_Jet20->Sumw2();
   hJetPt_Jet40->Sumw2();
   hJetPt_Jet60->Sumw2();
   hJetPt_Jet80->Sumw2();
   ...
   hNumEv_95_120_minbias->SetBinContent(1,numev_95_120_minbias);
   hNumEv_120_X_minbias->SetBinContent(1,numev_120_X_minbias);
   normalizeByBinWidth(hJetPt_minbias);
   normalizeByBinWidth(hJetPt_larger_minbias);
   ...
   hNumEv_95_120_minbias->Write();
   hNumEv_120_X_minbias->Write();
...
//         if (c_minbias->akPu3PF.trackMax[j]<4) continue; //Do we need this?
      ...
      hJetPt_minbias->Fill(data.leadingJetPt);
      hJetPt_larger_minbias->Fill(data.leadingJetPt);
      if(c_minbias->hlt.HLT_PAJet40_NoJetID_v1) {
        hJetPt_Jet40->Fill(data.leadingJetPt);
        hJetPt_larger_Jet40->Fill(data.leadingJetPt);
      }
      if(c_minbias->hlt.HLT_PAJet60_NoJetID_v1)
        hJetPt_Jet60->Fill(data.leadingJetPt);
      if(c_minbias->hlt.HLT_PAJet80_NoJetID_v1)
        hJetPt_Jet80->Fill(data.leadingJetPt);
      if(c_minbias->hlt.HLT_PAJet100_NoJetID_v1)
        hJetPt_Jet100->Fill(data.leadingJetPt);
      if(c_minbias->hlt.HLT_PAJet120_NoJetID_v1)
      ...
      if(data.leadingJetPt>=75. && data.leadingJetPt<95.)
         numev_75_95_minbias++;
      if(data.leadingJetPt>=95. && data.leadingJetPt<120.)
         numev_95_120_minbias++;
      ...
      for(int j=0;j<c_minbias->track.nTrk;j++) {
         ...
         double trkWeight = 1.;
//         std::cerr<<"--------" << std::endl;
//         std::cerr<<"  trkPt : " << c_minbias->track.trkPt[j] << std::endl;
//         std::cerr<<"  trkEta: " << c_minbias->track.trkEta[j] << std::endl;
//         std::cerr<<"  jetEt : " << data.leadingJetPt << std::endl;

         trkWeight = corr.getWeight(c_minbias->track.trkPt[j],c_minbias->track.trkEta[j],data.leadingJetPt);

//         std::cerr<<"  trkWeight: " << trkWeight << std::endl;
