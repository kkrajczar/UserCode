...
void CombineSpectra_FullTrackTrigger_Track30_EtaCM_P13_P18_NoOLDAlignmentRuns()
{
   ...
   bool doSave = false;
   TFile * f_output = new TFile("CombineSpectra_FullTrackTrigger_Track30_EtaCM_P13_P18_NoOLDAlignmentRuns.root","recreate");
...
//   double ptBins_part[] = {0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.4, 1.6, 1.8, 2.0, 2.2, 2.4, 3.2, 4.0, 4.8, 5.6, 6.4, 7.2, 9.6, 12.0, 14.4, 19.2, 24.0, 28.8, 35.2, 41.6, 48.0, 60.8, 73.6, 86.4, 103.6};
   ...
   //For trigger
   TH1D * hPartPt_FullTrack30_trkCorr = new TH1D("hPartPt_FullTrack30_trkCorr","hPartPt_FullTrack30_trkCorr",NumOfPtBins_part-1,&ptBins_part[0]);
...
//   TH1D * hPartPt_FullTrack50_trkCorr_trigCorr = new TH1D("hPartPt_FullTrack50_trkCorr_trigCorr","hPartPt_FullTrack50_trkCorr_trigCorr",ptBins.size()-1,&ptBins[0]);
   ...
   TH1D * hNumEv_FullTrack30_32_X_trigCorr = new TH1D("hNumEv_FullTrack30_32_X_trigCorr","hNumEv_FullTrack30_32_X_trigCorr",1,0.,1.);
//   TH1D * hNumEv_FullTrack50_trigCorr = new TH1D("hNumEv_FullTrack50_trigCorr","hNumEv_FullTrack50_trigCorr",1,0.,1.);

   hPartPt_FullTrack30_trkCorr->Sumw2();
...
//   hPartPt_FullTrack50_trkCorr_trigCorr->Sumw2();
   ...
   int numev_FullTrack30_32_X_trigCorr = 0;
//   int numev_FullTrack50_trigCorr = 0;

   for (int i=0;i<c_FullTrackTrigger->GetEntries();i++) {
//   for (int i=0;i<1000000;i++) {

      c_FullTrackTrigger->GetEntry(i);
      ...
      if(!(c_FullTrackTrigger->skim.phfPosFilter1 && c_FullTrackTrigger->skim.phfNegFilter1
          && c_FullTrackTrigger->skim.pBeamScrapingFilter
          && c_FullTrackTrigger->skim.pprimaryvertexFilter
          && c_FullTrackTrigger->skim.pVertexFilterCutGplus
          && TMath::Abs(c_FullTrackTrigger->evt.vz)<15.
          )
      ...
      //Jets
      for (int j=0;j<c_FullTrackTrigger->akPu3PF.nref;j++) {
         if (fabs(c_FullTrackTrigger->akPu3PF.jteta[j])>2.5) continue;
//         if (fabs(c_FullTrackTrigger->akPu3PF.jtpt[j])<30) continue;
//         if (c_FullTrackTrigger->akPu3PF.trackMax[j]<4) continue; //Do we need this?
         if (c_FullTrackTrigger->akPu3PF.rawpt[j]<15) continue;
         if (c_FullTrackTrigger->akPu3PF.jtpt[j]>data.leadingJetPt) {
            data.leadingJetPt = c_FullTrackTrigger->akPu3PF.jtpt[j];
            data.leadingJetEta = c_FullTrackTrigger->akPu3PF.jteta[j];
            data.leadingJetPhi = c_FullTrackTrigger->akPu3PF.jtphi[j];
            data.leadingJetTrackMax = c_FullTrackTrigger->akPu3PF.trackMax[j];
            data.leadingJetIt = j;
         }
      }
      ...
      int trackMult = 0;
      for(int j=0;j<c_FullTrackTrigger->track.nTrk;j++) {
         if(!((c_FullTrackTrigger->track.highPurity[j])
             && (fabs(c_FullTrackTrigger->track.trkDz1[j]/c_FullTrackTrigger->track.trkDzError1[j])<3)
             && (fabs(c_FullTrackTrigger->track.trkDxy1[j]/c_FullTrackTrigger->track.trkDxyError1[j])<3)
             && (c_FullTrackTrigger->track.trkPtError[j]/c_FullTrackTrigger->track.trkPt[j]<0.1)
            ))
            continue;
         trackMult++;
      }
      ...
      bool fullTrack30_trackAbove32 = false;
      double rapidityShift = 0.465; //eta_lab = eta_cm - y_NN
      for(int j=0;j<c_FullTrackTrigger->track.nTrk;j++) {
         if(!((c_FullTrackTrigger->track.trkPt[j]>0.2) 
             && (c_FullTrackTrigger->track.trkEta[j]<(1.8-rapidityShift))
             && (c_FullTrackTrigger->track.trkEta[j]>(1.3-rapidityShift))
             && (c_FullTrackTrigger->track.highPurity[j])
             && (fabs(c_FullTrackTrigger->track.trkDz1[j]/c_FullTrackTrigger->track.trkDzError1[j])<3)
             && (fabs(c_FullTrackTrigger->track.trkDxy1[j]/c_FullTrackTrigger->track.trkDxyError1[j])<3)
             && (c_FullTrackTrigger->track.trkPtError[j]/c_FullTrackTrigger->track.trkPt[j]<0.1)
           ))
            continue;            
           if(c_FullTrackTrigger->track.trkPt[j]>32.)
              { fullTrack30_trackAbove32 = true; break; }
      }
...
//         }
   ...
   hNumEv_FullTrack30_32_X_trigCorr->SetBinContent(1,numev_FullTrack30_32_X_trigCorr);
//   hNumEv_FullTrack50_trigCorr->SetBinContent(1,numev_FullTrack50_trigCorr);
   normalizeByBinWidth(hPartPt_FullTrack30_trkCorr);
   normalizeByBinWidth(hPartPt_FullTrack30_trkCorr_trigCorr);
...
*/
   f_output->cd();
   hPartPt_FullTrack30_trkCorr->Write();
   hPartPt_FullTrack30_trkCorr_trigCorr->Write();
