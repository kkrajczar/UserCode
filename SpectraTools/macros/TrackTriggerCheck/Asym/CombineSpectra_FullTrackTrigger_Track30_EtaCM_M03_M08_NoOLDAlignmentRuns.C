...
void CombineSpectra_FullTrackTrigger_Track30_EtaCM_M03_M08_NoOLDAlignmentRuns()
{
   ...
   bool doSave = false;
   TFile * f_output = new TFile("CombineSpectra_FullTrackTrigger_Track30_EtaCM_M03_M08_NoOLDAlignmentRuns.root","recreate");
...
//         if (c_FullTrackTrigger->akPu3PF.trackMax[j]<4) continue; //Do we need this?
      ...
      for(int j=0;j<c_FullTrackTrigger->track.nTrk;j++) {
         if(!((c_FullTrackTrigger->track.trkPt[j]>0.2) 
             && (c_FullTrackTrigger->track.trkEta[j]<(-0.3-rapidityShift))
             && (c_FullTrackTrigger->track.trkEta[j]>(-0.8-rapidityShift))
