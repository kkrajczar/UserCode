...
void CombineSpectra_FullTrackTrigger_Track12_EtaCM_M08_M13_NoOLDAlignmentRuns()
{
   ...
   bool doSave = false;
   TFile * f_output = new TFile("CombineSpectra_FullTrackTrigger_Track12_EtaCM_M08_M13_NoOLDAlignmentRuns.root","recreate");
...
//         if (c_FullTrackTrigger_Track12->akPu3PF.trackMax[j]<4) continue; //Do we need this?
      ...
      for(int j=0;j<c_FullTrackTrigger_Track12->track.nTrk;j++) {
         if(!((c_FullTrackTrigger_Track12->track.trkPt[j]>0.2) 
             && (c_FullTrackTrigger_Track12->track.trkEta[j]<(-0.8-rapidityShift))
