...
void CombineSpectra_minbias_EtaCM_P03_P08_NoOLDAlignmentRuns()
{
   ...
   bool doSave = true;
   TFile * f_output = new TFile("CombineSpectra_minbias_EtaCM_P03_P08_NoOLDAlignmentRuns.root","recreate");
...
//      hRecMultiplicity->Fill(trackMult);
      ...
      for(int j=0;j<c_minbias->track.nTrk;j++) {
         if(!((c_minbias->track.trkPt[j]>0.2) 
             && (c_minbias->track.trkEta[j]<(0.8-rapidityShift))
             && (c_minbias->track.trkEta[j]>(0.3-rapidityShift))
