...
void CombineSpectra_FullTrackTrigger_Track12_EtaCM_P13_P18_NoOLDAlignmentRuns()
{
   ...
   bool doSave = false;
   TFile * f_output = new TFile("CombineSpectra_FullTrackTrigger_Track12_EtaCM_P13_P18_NoOLDAlignmentRuns.root","recreate");
...
//   for (int i=0;i<1000000;i++) {
      ...
      if(c_FullTrackTrigger_Track12->evt.run<210676 || c_FullTrackTrigger_Track12->evt.run>211256) //211256: last pPb run (Pb goes to +eta)
         continue;
...
//         if (c_FullTrackTrigger_Track12->akPu3PF.trackMax[j]<4) continue; //Do we need this?
      ...
      for(int j=0;j<c_FullTrackTrigger_Track12->track.nTrk;j++) {
         if(!((c_FullTrackTrigger_Track12->track.trkPt[j]>0.2) 
             && (c_FullTrackTrigger_Track12->track.trkEta[j]<(1.8-rapidityShift))
             && (c_FullTrackTrigger_Track12->track.trkEta[j]>(1.3-rapidityShift))
