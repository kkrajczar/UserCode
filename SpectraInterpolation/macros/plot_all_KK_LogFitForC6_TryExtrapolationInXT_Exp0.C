...
void plot_all_KK_LogFitForC6_TryExtrapolationInXT_Exp0(Bool_t xt=kTRUE, Float_t expo=0.)
{
        SetStyle();
        gStyle->SetOptFile(0);
        gStyle->SetOptStat(0);
...
//	TGraphErrors* ratio_cdf_630_g = divide_graph_by_function(cdf_630_g,merge_fit);
	...
	TFile *output = new TFile("output_interpolation_KK_LogFitForC6_TryExtrapolationInXT_Exp0.root","RECREATE");
	h5020->Write();
