...
void plot_all_KK_LogFitForC6_TryExtrapolationInXT(Bool_t xt=kTRUE, Float_t expo=4.9)
{
        SetStyle();
        gStyle->SetOptFile(0);
        gStyle->SetOptStat(0);
...
//	TGraphErrors* ratio_cdf_630_g = divide_graph_by_function(cdf_630_g,merge_fit);
	...
	dumDirectInt->GetYaxis()->SetTitle("#sqrt{s}^{4.9} Ed^{3}#sigma/dp^{3}");
	dumDirectInt->Draw();
	h900->Draw("same");
	h1960->Draw("same");
	h7000->Draw("same");
	...
	for(Int_t ipt=0; ipt<=11; ipt++) {
		...
		dumXS[ipt]->GetXaxis()->SetRangeUser(0.5,10.0);
              	dumXS[ipt]->GetXaxis()->CenterTitle();
               	...
               	dumXS[ipt]->GetYaxis()->SetTitle("#sqrt{s}^{4.9} Ed#sigma^{3}/dp^{3}");
	...
	TFile *output = new TFile("output_interpolation_KK_LogFitForC6_TryExtrapolationInXT.root","RECREATE");
	h5020->Write();
