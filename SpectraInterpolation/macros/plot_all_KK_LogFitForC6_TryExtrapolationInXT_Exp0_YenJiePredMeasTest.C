...
void plot_all_KK_LogFitForC6_TryExtrapolationInXT_Exp0_YenJiePredMeasTest(Bool_t xt=kTRUE, Float_t expo=0.)
{
        SetStyle();
        gStyle->SetOptFile(0);
        gStyle->SetOptStat(0);
...
//	TGraphErrors* ratio_cdf_630_g = divide_graph_by_function(cdf_630_g,merge_fit);
	...
	TH1D *h5020 = new TH1D("h5020","",30400,0.005,0.05);
	TH1D *h5020_FromLogFit = new TH1D("h5020_FromLogFit","",30400,0.005,0.05);
	TH1D *h5020_To100GeV = new TH1D("h5020_To100GeV","",30400,0.005,0.05);
	TH1D *h5020_To100GeV_09_7 = new TH1D("h5020_To100GeV_09_7","",30400,0.005,0.05);
	TH1D *h5020_To100GeV_FromLogFit = new TH1D("h5020_To100GeV_FromLogFit","",30400,0.005,0.05);
	TH1D *h5020_To100GeV_09_7_FromLogFit = new TH1D("h5020_To100GeV_09_7_FromLogFit","",30400,0.005,0.05);
	...
	c5->cd(); //KK: Final low-pT fits
        ...
        //Use the 5.02 and 7 TeV "data" to predict the 2.76 points
        //Need to fit
        TGraph *gDirectInt_log_test[31000];
        TF1 *fitDirectInt_log_test[31000];
        TH1D *h2760_To100GeV_FromLogFit_test = new TH1D("h2760_To100GeV_FromLogFit_test","",30400,0.005,0.05);

        for(Int_t ipt=1; ipt<=30400; ipt++) {
            xs_log[0]=log10(h900->GetBinContent(ipt)); s_log[0]= log10(0.9);
            xs_log[1]=log10(h1960->GetBinContent(ipt)); s_log[1]=log10(1.96);
            xs_log[2]=log10(h5020_FromLogFit->GetBinContent(ipt)); s_log[2]=log10(5.02);
            xs_log[3]=log10(h7000->GetBinContent(ipt)); s_log[3]=log10(7.0);
            gDirectInt_log_test[ipt] = new TGraph(4,s_log,xs_log);
            fitDirectInt_log_test[ipt] = new TF1(Form("fitDirectInt_log_test%d",ipt),"pol2",-0.52288,0.85733);
            gDirectInt_log_test[ipt]->Fit(fitDirectInt_log_test[ipt],"REMWQ");//REMW");
            h2760_To100GeV_FromLogFit_test->SetBinContent(ipt,TMath::Power(10,fitDirectInt_log[ipt]->Eval(0.4409)));
        }

	TFile *output = new TFile("output_interpolation_KK_LogFitForC6_TryExtrapolationInXT_Exp0_YenJiePredMeasTest.root","RECREATE");
	h5020->Write();
	...
	h5020_FromLogFit->Write();
        ...
        h2760_To100GeV_FromLogFit_test->Write();
        h2760_EdTxt->Write();
